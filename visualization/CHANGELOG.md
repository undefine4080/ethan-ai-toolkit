...
- **代码逻辑可视化**（`scenes/code-logic/`）：第 8 个场景，专为晦涩代码的教学理解设计
  - **核心哲学**：不做静态流程图（只展示结构），做交互式执行模拟器（展示数据随时间的变化轨迹）
  - **适用对象**：循环/条件/回调密集的引擎代码、隐式状态变量多的控制逻辑、数据流多层变换的管道
  - **布局**：三栏等宽（变量指示器+循环状态机 / 消息时间线 / 步骤详情+代码片段）
  - **交互**：分步回放 + 自动播放 + 键盘快捷键（← → 空格 R）
  - **技术栈**：纯 HTML + Vanilla JS，零 CDN 依赖，深色 GitHub 风格主题
  - **与其他场景的关键差异**：
    - 不需要 `data.json`（数据直接内联为 JS 对象——执行步骤本身就是"数据"）
    - 不需要双 sub-agent（数据不是外部搜索采集的，由主 agent 设计步骤后交单个 sub-agent 实现）
    - 不需要 D3/CDN 库

### 场景路由表更新

- 路由表新增第 8 行：代码逻辑可视化，关键词覆盖 `代码`、`源码`、`逻辑`、`执行流程`、`循环`、`状态变量`、`引擎`
- Skill frontmatter 描述新增第(4)触发条件：用户要求可视化代码执行流程/控制流/数据变换路径

### 子 Agent 架构补充

- 新增「场景豁免」说明：代码逻辑场景不需要双 sub-agent，主 agent 自行设计步骤数据后启动单个实现 sub-agent

### 已知坑点（agent-loop.ts 实测记录）

- 代码片段括号闭合：每步 codeSnippet 写入后必须验证 `{` `}` 数量相等，否则看起来像 bug
- 字符串高亮正则与 HTML `<span>` 标签冲突：显示形如 `<span class="<span>">"kw"</span>` 的乱码 → 禁止字符串高亮，只做关键字和注释
- `codeSnippet` 数组末尾多余 `],` 导致 JS 语法错误 → 写入后用 `new Function()` 验证
- D3/静态流程图对代码理解无教学价值 → 做执行模拟器
- 步骤过多（>15）→ 控制在 8-12 个
- 缺少收束导致碎片化 → 最后一步必须给出完整时间线回顾面板
- 缺少隐式状态变量展示 → 左侧顶部加变量指示器，布尔值绿/红色高亮
- 占位数据（`foo`/`bar`）无意义 → 用真实文件名、真实工具名

### 产出物

- 完成 `scenes/code-logic/SCENE.md`（含技术栈、数据格式、实现要点、已知坑点）
- 产出配套 SOP 文档 `.ai/doc/code-visualization-sop.md`（完整的六步方法论）
